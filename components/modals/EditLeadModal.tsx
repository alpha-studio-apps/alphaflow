...
export default function EditLeadModal({ lead, onClose }: Props) {
  const [form, setForm] = useState({
    ...
    alpha_project: lead.alpha_project,
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-start sm:justify-end">
      ...
      <div className="relative z-10 w-full sm:w-[520px] max-h-[92dvh] sm:h-full bg-[#0d0d0d] sm:border-l border-t sm:border-t-0 border-[#242424] rounded-t-2xl sm:rounded-none overflow-y-auto flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a1a1a] sticky top-0 bg-[#0d0d0d] z-10 shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-white">Editar lead</h2>
            <p className="text-xs text-[#71717a] mt-0.5">{lead.first_name} {lead.last_name}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Empresa / Marca">
              <input value={form.company} onChange={e => set('company', e.target.value)} placeholder="Consultora MR" className={inputClass} />
            </Field>
            <Field label="Instagram">
              <input value={form.instagram} onChange={e => set('instagram', e.target.value)} placeholder="@usuario" className={inputClass} />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Servicio de interés">
              <input value={form.service_interested} onChange={e => set('service_interested', e.target.value)} className={inputClass} />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Valor estimado">
              <input type="number" value={form.estimated_value} onChange={e => set('estimated_value', e.target.value)} className={inputClass} />
            </Field>
          ...
          {/* Es cliente */}
          <label className="flex items-center gap-3 cursor-pointer bg-[#111111] border border-[#242424] rounded-lg px-4 py-3">
            <input
              type="checkbox"
              checked={form.is_client}
              onChange={e => set('is_client', e.target.checked)}
              className="w-4 h-4 accent-[#3B82F6]"
            />
            <div>
              <p className="text-sm text-white font-medium">Marcar como cliente activo</p>
              <p className="text-xs text-[#71717a]">Aparecerá en la sección Clientes</p>
            </div>
          </label>
